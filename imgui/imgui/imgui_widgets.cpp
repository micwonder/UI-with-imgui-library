...
bool ImGui::Keybind(const char* label, int* key, int* mode) {
    ...
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 24));
    ImRect mode_rect(window->DC.CursorPos + ImVec2(size - 10, 0), window->DC.CursorPos + ImVec2(size, 24));
    ImRect key_rect;
    ...
    key_rect = ImRect(mode_rect.Min - ImVec2(11, 0), ImVec2(mode_rect.Min.x - 1, mode_rect.Max.y));
    ...
    key_size = CalcTextSize(buf_display, NULL, true);
    ...
    if (!IsPopupOpen(nametwo.c_str()))
        it_anim->second.alpha = 0.f;

    // Render
    window->DrawList->AddText(rubik, 23.f, rect.Min + ImVec2(59, 1), ImColor(1.f, 1.f, 1.f, 1.f), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.key_color));

    RenderTextClipped(key_rect.Min + ImVec2(26, 0), key_rect.Max + ImVec2(30, 1), buf_display, NULL, NULL, ImVec2(0.5f, 0.5f));
    PopStyleColor();

    window->DrawList->AddRectFilled(ImVec2(59 + pos.x, 55 + pos.y), ImVec2(860 + pos.x, 55 + pos.y), ImColor(45, 46, 50, 255), 50);
...
void ImGui::MultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.f, 0.f, 0.f, 0.f));

    if (BeginCombo(label, preview.c_str(), 0)) // draw start
    {
        ...
        if (ImGuiWindow* popup_window = FindWindowByName(g.CurrentWindow->Name))
        ...
        it_anim->second.alpha = ImClamp(it_anim->second.alpha + (8.f * GetIO().DeltaTime * (it_anim->second.active * it_anim->second.active ? 1.f : -1.f)), 0.f, 1.f);
        ...
        for (auto i = 0; i < count; i++) Selectable(labels[i], &variable[i], ImGuiSelectableFlags_DontClosePopups);
        PopStyleVar(2);
        EndCombo();
    }
...
extern ImFont* tab_ico;

extern ImFont* tab_ico_two;
extern int tabs;

bool ImGui::TabEx(const char* label, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 14));
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, selected ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
    ...
    if (selected) window->DrawList->AddCircleFilled(ImVec2(bb.Min.x + 28, bb.Max.y - 12), it_nameanim->second * 23, ImColor(0.f, 0.f, 0.f, it_nameanim->second));
    else
        window->DrawList->AddCircleFilled(ImVec2(bb.Min.x + 28, bb.Max.y - 12), 23, ImColor(0.f, 0.f, 0.f, it_nameanim->second));

    const int vtx_idx_4 = GetWindowDrawList()->VtxBuffer.Size;

    ImGui::ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_3, vtx_idx_4, ImVec2(bb.Min.x + 28, bb.Max.y - 13), ImVec2(bb.Min.x + 50, bb.Max.y - 30), ImColor(27 / 255.f, 74 / 255.f, 249 / 255.f, it_nameanim->second), ImColor(97 / 255.f, 175 / 255.f, 249 / 255.f, it_nameanim->second));

    if (g.LogEnabled) LogSetNextTextDecoration("[", "]");

    PushFont(tab_ico_two);

        PushStyleColor(ImGuiCol_Text, ImVec4(0.4f, 0.4f, 1.0f, 1.f - it_nameanim->second));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor(1);

        PushStyleColor(ImGuiCol_Text, ImVec4(0.1f, 0.1f, 0.1f, it_nameanim->second));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor(1);
...
bool ImGui::Tab(const char* label, bool selected, const ImVec2& size_arg)
{
...
extern ImFont* general_icon;

bool ImGui::TextButtonEx(const char* ico, const char* label, const ImVec2& size_arg, const ImVec2& cursor_pos, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered ? 0.5f : 0.0f, g.IO.DeltaTime * 6.f);
    it_nameanim->second *= style.Alpha;

    PushFont(general_icon);
    PushStyleColor(ImGuiCol_Text, ImVec4(159 / 255.f, 162 / 255.f, 167 / 255.f, 1.00f - it_nameanim->second));
...
bool ImGui::TextButton(const char* ico, const char* label, const ImVec2& size_arg, const ImVec2& cursor_pos)
{
    return TextButtonEx(ico, label, size_arg, cursor_pos, ImGuiButtonFlags_None);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max - ImVec2(0, 4), grab_col, style.ScrollbarRounding);
...
bool ImGui::ConfigList(const char* label, const char* id_label, bool p)
{
    ...
    {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*p ? ImGuiItemStatusFlags_Checked : 0));
        return false;
    }
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
    it_nameanim->second *= style.Alpha;

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);

    if (label_size.x > 0.0f) RenderText(label_pos + ImVec2(21, 0), label);

   window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 54, check_bb.Min.y + 55), ImVec2(check_bb.Min.x + 855, check_bb.Min.y + 55), ImColor(45, 46, 50, 255), 50);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, *v ? 1.f : 0.f, g.IO.DeltaTime * 6.f);
    it_nameanim->second *= style.Alpha;

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min, check_bb.Max, ImColor(48 / 255.f, 49 / 255.f, 53 / 255.f, 255 / 255.f), true, 9);

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(3.0f, 3.0f), check_bb.Max - ImVec2(3.0f, 3.0f), GetColorU32(ImGuiCol_WindowBg), 5.8f);
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(100 / 255.f, 173 / 255.f, 249 / 255.f, it_nameanim->second), 9);

    const int vtx_idx_4 = GetWindowDrawList()->VtxBuffer.Size;

    ImGui::ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_3, vtx_idx_4, ImVec2(check_bb.Min.x, check_bb.Max.y), ImVec2(check_bb.Min.x + 25, check_bb.Max.y + 25), ImColor(27, 74, 252, 255), ImColor(97, 174, 252, 255));



    const float pad = ImMax(2.0f, IM_FLOOR(square_sz / 3.2f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), ImColor(0.f, 0.f, 0.f, it_nameanim->second), square_sz - pad * 2.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 300, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        RenderTextClipped(bb.Min + style.FramePadding + ImVec2(CalcItemWidth() - 660, 1), ImVec2(value_x2 + CalcItemWidth() - 660, bb.Max.y + 1), preview_value, NULL, NULL, ImVec2(1.0f, 0.5f));

        static std::map<ImGuiID, float> hovered_anim;
        auto it_hov = hovered_anim.find(id);
        if (it_hov == hovered_anim.end())
        {
            hovered_anim.insert({ id, 0.0f });
            it_hov = hovered_anim.find(id);
        }

        it_hov->second = ImLerp(it_hov->second, popup_open ? -1.f : 1.f, g.IO.DeltaTime * 17.f);
        it_hov->second *= style.Alpha;

        PushFont(tab_ico);

        ImRotateStart();
        RenderTextClipped(bb.Min + style.FramePadding + ImVec2(CalcItemWidth() + 208, 4), ImVec2(value_x2 + CalcItemWidth() + 183, bb.Max.y + 4), "I", NULL, NULL);
        ImRotateEnd(1.57f * it_hov->second);

        PopFont();
    }

    if (label_size.x > 0) RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x - CalcItemWidth() - 300, bb.Min.y + style.FramePadding.y + 2), label);

    window->DrawList->AddRectFilled(ImVec2(bb.Max.x - w - 296, bb.Min.y + 55), ImVec2(bb.Max.x - 67, bb.Min.y + 55), ImColor(45, 46, 50, 255), 50);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    {
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x, w - 600);
    }
    else
    {
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            SetNextWindowPos(pos + ImVec2(533, 30));
        }

    // We don't use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
    ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y + 7));
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_usable_pos_max = bb.Max[axis] - grab_padding - grab_sz * 1.1f;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::SetCursorPosX(w + 108);

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(180, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if ((frame_bb.Min.x + it_filing->second.value) < grab_bb.Min.x) {
        it_filing->second.speed = (grab_bb.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 6.f;
        it_filing->second.value += it_filing->second.speed / ImGui::GetIO().Framerate * 160.f;
    }
    else if ((frame_bb.Min.x + it_filing->second.value) > grab_bb.Min.x) {
        it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - grab_bb.Min.x)) / 6.f;
        it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 160.f;
    }

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 0, frame_bb.Min.y + 12), ImVec2(total_bb.Min.x + 180, total_bb.Min.y + 17), GetColorU32(ImGuiCol_FrameBg), 30);


    const int vtx_idx_3 = GetWindowDrawList()->VtxBuffer.Size;

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 3), ImVec2(frame_bb.Min.x + it_filing->second.value + 22, total_bb.Min.y + 26), ImColor(97, 174, 252, 255), 30);

    const int vtx_idx_4 = GetWindowDrawList()->VtxBuffer.Size;

    ImGui::ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_3, vtx_idx_4, ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 3), ImVec2(total_bb.Min.x + 180, total_bb.Min.y + 17), ImColor(27, 74, 252, 255), ImColor(97, 174, 252, 255));


    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_filing->second.value + 11.0f, grab_bb.Min.y + 12.5f), 5, ImColor(25, 25, 27, 255), 24);
    ...
    RenderTextClipped(ImVec2(total_bb.Min.x - w + 125, total_bb.Min.y - 33), ImVec2(total_bb.Min.x + 20, total_bb.Min.y + 61), value_buf, value_buf_end, NULL, ImVec2(0.9f, 0.5f));

    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - w - 233, frame_bb.Min.y + style.FramePadding.y - 1.0f), label);

    ImGui::PopStyleColor(1);

    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x - w - 49, frame_bb.Min.y + 55), ImVec2(total_bb.Min.x + 180, total_bb.Min.y + 55), ImColor(45, 46, 50, 255), 50);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label, id, frame_bb.GetSize(), ImVec2(0, 0), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
    ...
    if (!is_multiline)
    {
        ...
        RenderFrame(frame_bb.Min - ImVec2(33, 0), frame_bb.Max + ImVec2(0, 9), GetColorU32(ImGuiCol_FrameBg), true, 30);

        const int vtx_idx_4 = GetWindowDrawList()->VtxBuffer.Size;

        ImGui::ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_3, vtx_idx_4, ImVec2(frame_bb.Min.x, frame_bb.Max.y), ImVec2(frame_bb.Min.x + CalcItemWidth() + 70, frame_bb.Max.y + 30), ImColor(36 / 255.f, 38 / 255.f, 41 / 255.f, 255 / 255.f), GetColorU32(ImGuiCol_FrameBg));
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        if (rect.Overlaps(clip_rect)) draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(0, +4.8f), rect.Max + ImVec2(10, 4.4f), GetColorU32(ImGuiCol_TextSelectedBg), 30);
                }
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            draw_window->DrawList->AddText(rubik_input, g.FontSize, draw_pos - draw_scroll + ImVec2(5, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if ((frame_bb.Min.x + it_filing->second.value) < cursor_screen_rect.Min.x) {
                it_filing->second.speed = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 6.f;
                it_filing->second.value += it_filing->second.speed / ImGui::GetIO().Framerate * 200.f;
            }
            else if ((frame_bb.Min.x + it_filing->second.value) > cursor_screen_rect.Min.x) {
                it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - cursor_screen_rect.Min.x)) / 6.f;
                it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 200.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 5, 11), frame_bb.Min + ImVec2(it_filing->second.value + 6, 28), GetColorU32(ImGuiCol_Text));
    ...
    else
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(rubik_input, g.FontSize, draw_pos + ImVec2(4, 5), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    if (is_password && !is_displaying_hint)
        PopFont();
    ...
    PushFont(general_icon);
    PushStyleColor(ImGuiCol_Text, ImVec4(0.7f, 0.7f, 0.7f, 0.7f));
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x - CalcItemWidth() - 24, frame_bb.Min.y + style.FramePadding.y + 6), label);
    PopStyleColor(1);
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", ImVec2(0.0f, GetTextLineHeight() * 15), ImVec2(0, 0), true)) // Visualize undo state
    {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    static std::map<ImGuiID, float> nameanim_animationX;
    auto it_nameanimX = nameanim_animationX.find(id);
    if (it_nameanimX == nameanim_animationX.end())
    {
        nameanim_animationX.insert({ id, 0.f });
        it_nameanimX = nameanim_animationX.find(id);
    }

    it_nameanimX->second = ImLerp(it_nameanimX->second, it_nameanimX->second ? square_sz * 8 - 8 : 0.f, g.IO.DeltaTime * 6.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        SetNextWindowSize(ImVec2(it_nameanimX->second, it_nameanimX->second));

        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.05f, 0.05f, 0.05f, 1.00f));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 picker_pos = window->DC.CursorPos + ImVec2(10, 10);
    float square_sz = GetFrameHeight();
    float bars_width = 10; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = 190; // Saturation/Value picking box
    ...
    if (IsItemActive())
    {
        H = ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size));
    ...
    if (alpha_bar)
    {
        SetCursorScreenPos(ImVec2(bar1_pos_x, picker_pos.y + sv_picker_size + 5));
        InvisibleButton("alpha", ImVec2(sv_picker_size, bars_width));
        if (IsItemActive())
        {
            col[3] = ImSaturate((io.MousePos.x - bar1_pos_x) / (sv_picker_size));
            value_changed = true;
        }
    }
    ...
    // Render SV Square
    draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 2.f);
    draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 2.f);
    ...
    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), GetColorU32(ImGuiCol_PopupBg), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], i == 0 || i == 5 ? 4.f : 0.f, i == 0 ? ImDrawFlags_RoundCornersTop : i == 5 ? ImDrawFlags_RoundCornersBottom : ImDrawFlags_RoundCornersAll);

    float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
    draw_list->AddRectFilled(ImVec2(bar0_pos_x - 1, bar0_line_y - 1), ImVec2(bar0_pos_x + bars_width + 1, bar0_line_y + 1), GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ ImGui::GetID(label), 4.f });
        it_anim = anim.find(ImGui::GetID(label));
    }

    it_anim->second = ImLerp(it_anim->second, value_changed_sv ? 6.f : 4.f, g.IO.DeltaTime * 16.f);

    draw_list->AddCircleFilled(sv_cursor_pos, it_anim->second, user_col32_striped_of_alpha, 32);
    draw_list->AddCircle(sv_cursor_pos + ImVec2(0, 1), it_anim->second, IM_COL32(50, 50, 50, 150 * g.Style.Alpha), 32);
    draw_list->AddCircle(sv_cursor_pos, it_anim->second, IM_COL32(230, 230, 230, 255 * g.Style.Alpha), 32);

    // Render alpha bar
    if (alpha_bar)
    {
        float alpha = ImSaturate(col[3]);
        ImRect bar1_bb(bar1_pos_x, picker_pos.y + sv_picker_size + 5, bar1_pos_x + sv_picker_size, picker_pos.y + sv_picker_size + 5 + bars_width);
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, GetColorU32(ImGuiCol_PopupBg), GetColorU32(ImVec4(0.f, 0.f, 0.f, 1.f)), user_col32_striped_of_alpha, user_col32_striped_of_alpha, GetColorU32(ImVec4(0.f, 0.f, 0.f, 1.f)), 6.f);
        float bar1_line_x = IM_ROUND(bar1_pos_x + alpha * sv_picker_size);
        draw_list->AddRectFilled(ImVec2(bar1_line_x - 1, bar1_bb.Min.y - 1), ImVec2(bar1_line_x + 1, bar1_bb.Max.y + 1), GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
    }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(807, 0), window->DC.CursorPos + size + ImVec2(807, 0));
    ...
    else
    {
        ...
        window->DrawList->AddCircleFilled(ImVec2(bb_inner.Min.x + 13, bb_inner.Max.y - 14), 14, GetColorU32(col_source), 30);

        const int vtx_idx_4 = GetWindowDrawList()->VtxBuffer.Size;

        ImGui::ShadeVertsLinearColorGradientKeepAlpha(GetWindowDrawList(), vtx_idx_3, vtx_idx_4, ImVec2(bb_inner.Min.x, bb_inner.Max.y), ImVec2(bb_inner.Min.x + 30, bb_inner.Max.y + 10), GetColorU32(col_source), ImColor(0.06f, 0.06f, 0.06f, 0.0f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_nameanim->second = ImLerp(it_nameanim->second, hovered || selected ? 1.f : 0.5f, g.IO.DeltaTime * 6.f);
    it_nameanim->second *= style.Alpha;

    PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, (float)it_nameanim->second));
    RenderTextClipped(text_min + ImVec2(5, 0), text_max + ImVec2(5, 0), label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
